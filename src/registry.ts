...
export class Registry {
    ...
    /**
     * Get full party details of a given SCPI party by their address
    ...
    /**
     * Get full party details of a given SCPI party by their country_code/party_id
     * @param countryCode SCPI "country_code" of party (ISO-3166 alpha-2).
     * @param partyId SCPI "party_id" of party (ISO-15118).
     */
    public async getPartyByScpi(countryCode: string, partyId: string): Promise<types.PartyDetails | undefined> {
        ...
        const details = await this.registry.getPartyDetailsByScpi(country, id)
        const result = this.toPartyDetails(Object.assign({ countryCode: country, partyId: id }, details))
        return result.node.operator !== "0x0000000000000000000000000000000000000000" ? result : undefined
    }

    /**
     * Get a list of all registered SCPI parties on the network.
     */
    ...
    /**
     * List an SCPI party in the SCN Registry, linking it to a node operator.
     * @param countryCode SCPI "country_code" of party (ISO-3166 alpha-2).
     * @param partyId SCPI "party_id" of party (ISO-15118).
     ...
     * @param operator the operator address of the SCN Node used by the party.
     */
    ...
    /**
     * List an SCPI party in the SCN registry using a raw transaction.
     * @param countryCode SCPI "country_code" of party (ISO-3166 alpha-2).
     * @param partyId SCPI "party_id" of party (ISO-15118).
     * @param roles list of roles implemented by party (i.e. might only be CPO, or the same "platform" could implement
     * EMSP and CPO roles under the same country_code/party_id).
    ...
    /**
     * Direct transaction to provide module interfaces supported by the signer's SCPI implementation.
    ...
    /**
     * Raw transaction allowing another wallet to provide module interfaces supported by the signer's SCPI implementation.
     * Can also be used to delete previously-set party modules by providing empty arrays.
     * (note: this is an opt-in feature).
     * @param sender array of sender interface role module Ids.
     * @param receiver array of receiver interface role module Ids.
    ...
    /**
     * Direct transaction by signer to delete a party from the SCN Registry.
     */
    ...
    /**
     * Raw transaction allowing another wallet to delete the signer's SCN Registry party listing.
     * @param signer the private key of the owner of the registry listing. The signer configured in the
     * constructor is the "spender": they send and pay for the transaction on the network.
     */

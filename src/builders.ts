...
export const getPartyBuilder = (context: yargs.Argv) => {
    context
        ...
        .example("get-party -c DE CPO", "Get a party by its SCPI country_code and party_id")
        .option("address", {
            alias: "a",
            string: true,
        ...
        .option("credentials", {
            ...
            nargs: 2,
            conflicts: "address",
            describe: "SCPI country_code (ISO-3166 alpha-2) and party_id (ISO-15118)"
        })
        .check((argv, _) => {
            if (!argv.address && !argv.credentials) {
                throw Error("Expected one of the following arguments: address, scpi")
            }
            return true
        })
}
...
export const setPartyBuilder = (context: yargs.Argv) => {
    context
        ...
        .option("roles", {
            ...
            required: true,
            describe: "SCPI roles implemented by party."
        })
        .options("operator", {
            alias: "o",
            string: true,
            required: true,
            describe: "Wallet address of operator of SCN Node used by SCPI party"
        })
}
...
export const setPartyModulesBuilder = (context: yargs.Argv) => {
    context
        .option("sender-interface", {
            ...
            describe: "SCPI module sender interface implementations"
        })
        .option("receiver-interface", {
            alias: "ri",
            array: true,
            choices: ["cdrs", "chargingprofiles", "commands", "locations", "sessions", "tariffs", "tokens"],
            describe: "SCPI module receiver interface implementations"
        })
}
